...
const nameList = Deno.readTextFileSync("ngram-idioms/ignore-family-name.lst")
  .split("\n");
const sexualList = Deno.readTextFileSync(
  "ngram-idioms/inappropriate-words-ja/Sexual.txt",
)
  .split("\n");
const ignoreList = Deno.readTextFileSync(
  "ngram-idioms/ignore" + idiomLength + ".lst",
).split(
  "\n",
);
...
const baseDir = `kanji-${idiomLength}-${thresholdName}`;
Deno.mkdirSync(baseDir, { recursive: true });
for (let level = 1; level <= 10; level++) {
  const idioms = await listup(level);
