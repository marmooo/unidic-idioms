...
for (var i=1; i<=9; i++) {
  var idioms = fs.readFileSync('kanji-2-3999/' + i + '.arr');
...
for (var i=1; i<=9; i++) {
  var idioms = fs.readFileSync('kanji-3-3999/' + i + '.arr');
  stream.write(`let i${i} = Array.from('${idioms.slice(0, -1)}');\n`);
}
stream.end();
...
for (var i=1; i<=9; i++) {
  var idioms2 = fs.readFileSync('kanji-2-3999/' + i + '.arr');
  var idioms3 = fs.readFileSync('kanji-3-3999/' + i + '.arr');
